...
class Metric_dataframe():
    ...
    def load_flow_npz(self, flow_npz_path, seq, scale_k):
        ...
        for basename in tqdm(flow_npz.files):
            flow = self.flow[basename]
            # normalized
            H, W, _ = flow.shape
            self.flow_mag = np.sqrt((flow[:,:,0]/(scale_k * W))**2 + (flow[:,:,1]/(scale_k * H))**2)
            # print(flow_mag.max(), flow_mag.min())
            self.flow_mag = np.clip(self.flow_mag, None, 1)       
        return self.flow_npz

    def preprocess_each_seq(self, **kwargs):
        ...
        if 'masked_SSIM' in self.metric_types:
            self.flow_npz = self.load_flow_npz(**kwargs)
    ...
    def calc_metric(self, metric_name, output_image, gt_image, **kwargs):
        if metric_name == 'PSNR':
            ...
            self._value = ssim(output_image, gt_image, channel_axis=2, gaussian_weights=True, sigma=1.5, use_sample_covariance=False,
                        data_range=255.0)
            ...
            _, ssim_map = ssim(output, gt, gaussian_weights=True, sigma=1.5, use_sample_covariance=False,
                    data_range=255.0, gradient=False, full=True)
            

            #[TODO]
            # make mask and calculate pixel wise multiplication 
...
def calc_metrics_new(seq_dict: dict, save_dir: str, metric_type_list: List[str], **kwargs) -> None:
    ...
    for seq, (output_path_list, gt_path_list) in seq_dict.items():
        ...
        for output_path, gt_path in zip(tqdm(output_path_list), gt_path_list):
            ...
            metrics.calc_metric_all(output_image=output, gt_image=gt)
...
if __name__ == '__main__':
    ...
    seq_select = 'all'
    save_dir = '../STDAN_modified/exp_log/train/%s/visualization/c1200_out_debug' % exp
    ...
    # calc_ssim_map(seq_dict, save_dir, grayscale=True, mode=mode)
    calc_metrics_new(seq_dict, save_dir, metric_type_list)
